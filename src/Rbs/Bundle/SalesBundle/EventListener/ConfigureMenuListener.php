...
class ConfigureMenuListener extends ContextAwareListener
{
    ...
    public function onMenuConfigureMain(ConfigureMenuEvent $event)
    {
        ...
        if ($this->user->getUserType() != User::AGENT) {
            if ($this->authorizationChecker->isGranted(array('ROLE_SUPER_ADMIN','ROLE_DEPO_MENU_ACCESS','ROLE_ORDER_VIEW', 'ROLE_ORDER_CREATE', 'ROLE_ORDER_EDIT', 'ROLE_ORDER_APPROVE', 'ROLE_ORDER_CANCEL', 'ROLE_FEED_ORDER_MANAGE'))) {
            ...
            if ($this->authorizationChecker->isGranted(array('ROLE_SUPER_ADMIN','ROLE_HATCHERY_MENU_ACCESS', 'ROLE_CHICK_ORDER_MANAGE'))) {
                ...
                if ($this->authorizationChecker->isGranted(array('ROLE_CHICK_ORDER_MANAGE','ROLE_DEPO_USER'))) {
                    $menu3->addChild('Orders', array('route' => 'order_chick_lists'))->setAttribute('icon', 'fa fa-th-list');
                }
        ...
        if ($this->user->getUserType() == User::USER or $this->user->getUserType() == User::ZM) {
            ...
            if ($this->authorizationChecker->isGranted(array('ROLE_DELIVERY_MANAGE'))) {
                $sp3 = true;
                $menu['Sales']->addChild('Turck In/Out', array('route' => 'truck_info_in_out_list'))
                    ->setAttribute('icon', 'fa fa-th-list');
                if ($this->isMatch('truck_info_in_out_list')) {
                    $menu['Sales']->getChild('Turck In/Out')->setCurrent(true);
                }
            }

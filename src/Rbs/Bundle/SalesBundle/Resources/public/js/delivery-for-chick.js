var Delivery = function()
{
    ...
    function total(element, totalElement) {
        var sum = 0;
// iterate through each td based on class and add the values
    ...
    function init()
    {
        ...
        $('.damage-qty').on('keyup', function () {
            var element = $(this);
            var thisValue = $(this).val();
            var totalDeliveryElement = $(this).closest('tr').find('.totalDeliveryQty');
            var item_qty = $(this).closest('tr').find('.item-qty').text();
            if(thisValue===''){
                element.val(parseInt(0));
            }else {
                element.val(parseInt(thisValue));
            }

            if(parseInt(item_qty)>=parseInt($(this).val())){
                totalDeliveryElement.text(parseInt(item_qty)- parseInt($(this).val()));
            }else {
                alert('Actual Quantity not available');
                element.val(parseInt(0));
                totalDeliveryElement.text(parseInt(item_qty));
            }

            var sum = 0;
            $('.allDamageQty').each(function() {
            ...
            $('.total_damage').text(sum);

            var deliverySum = 0;
// iterate through each td based on class and add the values
            $('.totalDeliveryQty').each(function() {

                var value = $(this).text();
                // add only if the value is number
                if(!isNaN(value) && value.length != 0) {
                    deliverySum += parseFloat(value);
                }
            });

            $('.grandTotalDeliveryQty').text(deliverySum);

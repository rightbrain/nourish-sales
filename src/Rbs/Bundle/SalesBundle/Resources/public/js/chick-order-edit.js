var Order = function()
{
    ...
    function findStockItem(item, index) {
        ...
        $.ajax({
            ...
            url: Routing.generate('find_stock_item_depo_ajax', {
                ...
                depo: $('#order_depo').val(),
                order: $('#order_id').val(),
                order_date: $('#order_created_at').val()
            }),
            //data: "item=" + item + "&agent=" + $('#order_agent').val() + "&depoId=" + $('#order_depo').val() + "&orderId=" + $('#order_id').val() ? $('#order_id').val() : '',
            dataType: 'json',
            success: function (response) {
    ...
    function addedOrderItemQty() {

        $('tbody.tags').find('tr').each(function(index, elm){

             var quantity=  $("#order_orderItems_" + index + "_quantity").val();
             $(".added_qty_" + index).val(parseFloat(quantity));
    ...
    function formValidateInit()
    {
        ...
        form.submit(function(){
            var isFormValid = true,
                ...
                payments = $('#payments'),
                order_depot = $('#order_depo'),
                previous_order_depot = $('.previous_depot_id'),
                qtyError = false,
                priceError = false,
            ...
            orderItem.find('tr').each(function(index, e){
                ...
                var addedQty = elm.find('td:eq(7)').find('input[type=hidden]').val();

                if(order_depot.val() !== previous_order_depot.val()){
                    addedQty = 0;
                }
                ...
                if (
                    ...
                    (parseInt(qty.find('input').val()) > (parseInt(stock)+parseInt(addedQty)))
                ) {
